...
def test_an_out_of_order_revision_is_coloured_and_explained(tmp_path: Path):
    ...
    try:
        ...
        assert bad.comment is None, "a sheet is not read by hovering it"
        # What red means is on the sheet, beside the grid that uses it.
        legend = [c.value for col in ws.iter_cols(min_col=5) for c in col]
        assert "Revision Out Of Order" in legend
        assert any(isinstance(v, str) and "approval starts at A" in v
                   for v in legend)
    finally:
        wb.close()
...
def _shop(label: str, stage: str, round_no: str,
          marks: dict[str, tuple[str, str, str]]) -> IssueEntry:
    """An issue whose members carry a revision, a quantity and a cut length."""
    members: "OrderedDict[str, dict[str, str]]" = OrderedDict()
    for mark, (rev, qty, length) in marks.items():
        members[mark] = {"name": mark, "rev": rev, "zone": "1",
                         "category": "Part", "qty": qty, "length": length}
    return IssueEntry(label=label, stage=stage, round_no=round_no, members=members)


def test_a_length_reads_back_as_inches():
    assert parse_length_inches("3'-11 5/8\"") == 47.625
    assert parse_length_inches("8'-7 15/16\"") == 103.9375
    assert parse_length_inches("29.00") == 29.0
    assert parse_length_inches("A500-GR.C") is None
    assert parse_length_inches("") is None


def test_inches_read_back_the_way_the_shop_writes_them():
    assert format_inches(47.625) == "3'-11 5/8\""
    assert format_inches(103.9375) == "8'-7 15/16\""
    assert format_inches(1.8125, signed=True) == "+0'-1 13/16\""
    assert format_inches(-2.125, signed=True) == "-0'-2 1/8\""


def test_a_quantity_that_moves_is_reported_with_the_issue_that_moved_it():
    state = ChainState()
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "3", "3'-11 5/8\"")}))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", {"P1": ("1", "2", "3'-11 5/8\"")}))
    changes = build_chain(state).spec_changes
    assert len(changes) == 1
    change = changes[0]
    assert (change.field, change.old, change.new) == ("Qty", "3", "2")
    assert change.label == "IFF-2" and change.since == "IFF-1"


def test_a_length_that_moves_says_by_how_much():
    state = ChainState()
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "1", "0'-8 5/16\"")}))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", {"P1": ("1", "1", "0'-10 1/8\"")}))
    change = build_chain(state).spec_changes[0]
    assert change.field == "Length"
    assert change.delta == "+0'-1 13/16\"", "the shop cuts in feet and inches"


def test_nothing_is_reported_when_nothing_moved():
    """Most of a re-issue is unchanged, and saying so is the point."""
    state = ChainState()
    same = {"P1": ("0", "3", "3'-11 5/8\""), "P2": ("0", "1", "2'-0\"")}
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", same))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", same))
    assert build_chain(state).spec_changes == []


def test_a_member_that_skips_an_issue_has_not_changed_twice():
    """Compared against the last issue that carried a value, not the one before."""
    state = ChainState()
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "3", "2'-0\"")}))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", {"P9": ("0", "1", "1'-0\"")}))
    state.add_issue(_shop("IFF-3", STAGE_IFF, "3", {"P1": ("1", "4", "2'-0\"")}))
    changes = [c for c in build_chain(state).spec_changes if c.member_name == "P1"]
    assert [(c.old, c.new, c.label) for c in changes] == [("3", "4", "IFF-3")]


def test_the_two_numbers_get_a_sheet_each(tmp_path: Path):
    """How many to make and how long to cut are read for different reasons."""
    state = ChainState(project="P")
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "3", "3'-11 5/8\"")}))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", {"P1": ("1", "2", "4'-0\"")}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")

    wb = load_workbook(out)
    try:
        assert "Qty" in wb.sheetnames and "Length" in wb.sheetnames

        qty = wb["Qty"]
        assert [c.value for c in qty[3]] == [
            "Member Name", "Zone", "Sequence", "IFF-1", "IFF-2",
            "Changed", "What Changed"]
        row = [c.value for c in qty[4]]
        assert row[0] == "P1"
        assert row[3] == 3 and row[4] == 2, "a quantity is written as a number"
        assert row[5] == "Qty" and row[6] == "3 to 2 at IFF-2"
        moved = qty.cell(row=4, column=5)
        assert moved.fill.fgColor.rgb.endswith("FFF2CC"), "the changed cell is amber"
        assert moved.comment is None, "a sheet is not read by hovering it"

        length = wb["Length"]
        row = [c.value for c in length[4]]
        assert row[3] == "3'-11 5/8\"" and row[4] == "4'-0\""
        assert row[5] == "Length"
        assert row[6] == "3'-11 5/8\" to 4'-0\" (+0'-0 3/8\") at IFF-2"
    finally:
        wb.close()


def test_a_category_gets_no_column_for_an_issue_it_was_not_in(tmp_path: Path):
    """Parts are detailed for fabrication, so no part is in an approval round.

    A column of blanks across every part reads as a package that dropped them
    all at that issue.
    """
    state = ChainState(project="P")
    state.add_issue(issue("IFA-1", STAGE_IFA, "1", {"A1": "A"}))
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "1", "2'-0\"")}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")

    wb = load_workbook(out)
    try:
        assert [c.value for c in wb["History - Part"][3]][:4] == [
            "Member Name", "Zone", "IFF-1", None]
        # The assemblies were in both, and the release left A1 behind.
        assert [c.value for c in wb["History - Assembly"][3]][:4] == [
            "Member Name", "Zone", "IFA-1", "IFF-1"]
    finally:
        wb.close()


def test_the_issue_that_dropped_a_member_keeps_its_column(tmp_path: Path):
    """That column is carrying the D - it is the whole record of the drop."""
    state = ChainState(project="P")
    state.add_issue(issue("IFA-1", STAGE_IFA, "1", {"A1": "A", "A2": "A"}))
    state.add_issue(issue("IFA-2", STAGE_IFA, "2", {"A1": "B"}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")
    ...
    try:
        ws = wb["History - Assembly"]
        assert [c.value for c in ws[3]][:4] == [
            "Member Name", "Zone", "IFA-1", "IFA-2"]
        dropped = next(r for r in ws.iter_rows(min_row=4, values_only=True)
                       if r[0] == "A2")
        assert dropped[3] == "D"
    finally:
        wb.close()
...
def test_a_package_with_lengths_but_no_quantities_gets_one_sheet(tmp_path: Path):
    """Each sheet stands on its own number."""
    state = ChainState(project="P")
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "", "2'-0\"")}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")

    wb = load_workbook(out)
    try:
        assert "Length" in wb.sheetnames and "Qty" not in wb.sheetnames
    finally:
        wb.close()


def test_a_part_that_never_moved_says_so_in_words(tmp_path: Path):
    """Most of a re-issue is unchanged and the sheet has to say which part is."""
    state = ChainState(project="P")
    same = {"P1": ("0", "3", "3'-11 5/8\"")}
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", same))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", same))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")
    ...
    wb = load_workbook(out)
    try:
        for name in ("Qty", "Length"):
            row = [c.value for c in wb[name][4]]
            assert row[5] == "No change" and not row[6]
    finally:
        wb.close()


def test_only_the_parts_carrying_them_get_a_row(tmp_path: Path):
    """An assembly has no cut length, and a row of blanks answers nothing."""
    state = ChainState(project="P")
    entry = _shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "3", "2'-0\"")})
    entry.members["Assembly::A1"] = {"name": "A1", "rev": "0", "zone": "1",
                                     "category": "Assembly"}
    entry.__post_init__()
    state.add_issue(entry)
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")
    ...
    wb = load_workbook(out)
    try:
        names = [r[0] for r in wb["Qty"].iter_rows(min_row=4, values_only=True)]
    finally:
        wb.close()
    assert "P1" in names and "A1" not in names


def test_an_approval_package_gets_no_such_sheet(tmp_path: Path):
    """An approval title block carries no quantities; an empty sheet is a question."""
    state = ChainState(project="P")
    state.add_issue(issue("IFA-1", STAGE_IFA, "1", {"A1": "A"}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")
    wb = load_workbook(out)
    try:
        assert "Qty" not in wb.sheetnames and "Length" not in wb.sheetnames
    finally:
        wb.close()


def test_the_change_log_lists_what_moved(tmp_path: Path):
    """One filter answers "what actually changed in this issue"."""
    state = ChainState(project="P")
    state.add_issue(_shop("IFF-1", STAGE_IFF, "1", {"P1": ("0", "3", "2'-0\"")}))
    state.add_issue(_shop("IFF-2", STAGE_IFF, "2", {"P1": ("1", "3", "2'-6\"")}))
    out = write_tracker(build_chain(state), tmp_path / "T.xlsx")

    wb = load_workbook(out)
    try:
        rows = [r for r in wb["Change Log"].iter_rows(min_row=4, values_only=True)
                if r[3] == "Length changed"]
    finally:
        wb.close()
    assert len(rows) == 1
    assert rows[0][6] == "P1" and rows[0][7] == "2'-0\"" and rows[0][8] == "2'-6\""
